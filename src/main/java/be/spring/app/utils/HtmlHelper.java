...
public class HtmlHelper {
    ...
    private static String getBtn(String aClazz, String clazz, String url) {
        return String.format
                ("<a href='%s' data-toggle='tooltip' data-placement='top' class='btn btn-default %s'><span class='%s' aria-hidden=\"true\"></span></a>", url, aClazz, clazz);
    }

    private static String getBtn(String aClazz, String clazz, String url, String title) {
        return String.format
                ("<a href='%s' title='%s' data-toggle='tooltip' data-placement='top' class='btn btn-default %s'><span class='%s' aria-hidden=\"true\"></span></a>", url, title, aClazz, clazz);
    }
    ...
    public Map<String, String> getMatchesButtons(Match match, boolean isAdmin, Locale locale) {
        ...
        if (isAdmin) {
            btns.append(getBtn(EMPTY, EDIT, String.format("changeMatch.html?matchId=%s", match.getId()),
                    messageSource.getMessage("title.changeMatchResult", null, locale)))
                    .append(getBtn(DELETE_CLASS, DELETE, String.format("deleteMatch.html?matchId=%s", match.getId()),
                            messageSource.getMessage("title.deleteMatch", null, locale)));
        }

        String googleLink = match.getHomeTeam().getAddress().getGoogleLink();
        if (googleLink != null)
            btns.append(getBtn(MAP_CLASS, MAP, googleLink, messageSource.getMessage("title.matchLocation", null,
                    locale)));

        if (!match.getGoals().isEmpty())
            btns.append(getBtn(DETAILS_CLASS, DETAILS, "details" + match.getId(), messageSource.getMessage("title" +
                    ".matchDetails", null, locale)));

        btns.append(getBtn(DOODLE_CLASS, DOODLE, String.format("getDoodle.html?matchId=%s", match.getId()),
                messageSource.getMessage("title.matchDoodle", null, locale)));

        if (match.getMotmPoll() != null) {
            btns.append(getBtn(MOTM_CLASS, MOTM, "motm" + match.getId().toString(), messageSource.getMessage("title" +
                    ".manOfTheMatchPoll", null, locale)));
        }

        m.put(HTML_ACTIONS, btns.toString().isEmpty() ? messageSource.getMessage("text.noActions", null, locale) :
                wrapIntoBtnGroup(btns.toString()));
        return m;
    }
    ...
    public Map getTeamButtons(Team team, boolean isAdmin, Locale locale) {
        ...
        if (isAdmin) {
            btns.append(getBtn(EMPTY, EDIT, String.format("editTeam.html?teamId=%s", team.getId()), messageSource
                    .getMessage("title.editTeam", null, locale)))
                    .append(getBtn(DELETE_CLASS, DELETE, String.format("deleteTeam.html?teamId=%s", team.getId()),
                            messageSource.getMessage("title.deleteTeam", null, locale)));
        }
